		...
		<div class="container">
			<div class="inner-content">
				...
				<ul>
					...
					<li><a href="#" title="">회원 매물요청 보기</a></li>
				</ul>
			</div>
		</div>
	</div>
										...
										<c:if test="${not empty printlist}">
											...
											<c:forEach var="printlist" items="${printlist}">
												...
												<tr class="active">
													...
													<c:if test="${printlist.pCpId == 0}">
														<c:set var="rownum" value="${printlist.ridx}" /> 
														<c:set var="replynum" value="${0}" /> 
														${rownum}
													</c:if>
													<c:if test="${printlist.pCpId != 0}">
														<c:if test="${printlist.userId == sbAgentDo.userId}">
													...
													<td align="center"><a
														href="${pageContext.request.contextPath}/property/askforDetail/${sb:getIdStr(printlist.cpId)}">
															${printlist.conTitle} </a></td>
													<td align="center">${printlist.address}</td>
													<td style="padding-left: 2em;">${printlist.price}</td>
