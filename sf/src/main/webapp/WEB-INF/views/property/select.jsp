...
<script type='text/javascript'>
	$(document).ready( function(){
			...
			if((!selectform.propid.value && dupcheck > 0)||(!!selectform.propid.value && dupcheck === 0)) {
				...
				opener.document.askfordetail.propId.value
			}
	...
	function goPage(pg) {
        $(location).attr('href', '${pageContext.request.contextPath}/property/select/'+$("#propsid").val()+'/'+$("#propid").val()+'/'+pg);
    }
		...
		if(checkboxes[idx].checked){
			if(dup === 0){
				$.ajax(
					{
						type:"POST",
						url: '${pageContext.request.contextPath}/property/addtoPropsId/'+propvalue,
						dataType:"text",
			...
			$.ajax(
					{
						type:"POST",
						url: '${pageContext.request.contextPath}/property/deleteFromPropsId/'+propvalue,
						dataType:"text",
						success:function(datum){
							if (datum !== "abc"){
								alert("삭제됨");
	...
	function setpropsid( id ){
		...
		if ( !!selectform["print"].value ){				// added by sf on October 11
			address = String(selectform["print"].value);
		} else {											// added by sf on October 11
			for (i=0; i<selectform.myCheck.length; i++){
		...
		opener.document.askfordetail.propId.value = id;
		
		if (count !== "0")
			opener.document.askfordetail.addresses.value = address + " 외 " + count + "건";		// added by sf October 10
		else if( !id )
			opener.document.askfordetail.addresses.value = "";
		else
			opener.document.askfordetail.addresses.value = address;		// added by sf October 10
	...
	<div class="properties-content properties-grid">
		<div class="container ">
			...
			<form id="selectform" name="selectform" method="post" class="property-form" role="form" onsubmit="setpropsid(selectform.propid.value)">
